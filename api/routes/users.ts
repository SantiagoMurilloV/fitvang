...
const createUserSchema = z.object({
  ...
  // password opcional: si no se manda se genera y se devuelve al admin
...
usersRouter.post('/', requireAdmin, zValidator('json', createUserSchema), async (c) => {
  ...
  await sendPushToUser(u.id, 'bienvenida', {
...
usersRouter.get('/:id', async (c) => {
  ...
  if (me.rol === 'user' && me.sub !== id) return c.json({ error: 'forbidden' }, 403);
